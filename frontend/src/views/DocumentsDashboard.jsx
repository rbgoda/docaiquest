...
function exportCsv(d) {
  ...
  const rows = [
    ["DocAIQ · Documents workspace overview"], [],
    ["Metric", "Value"],
    ...
    try {
      ...
      if (!c.personalData) {
        const ok = await confirmDialog({
          ...
          body: "Your documents may contain personal or special-category (health) data. DocAIQ processes them to extract and answer questions; redacted text may be sent to AI providers. By continuing you acknowledge this and consent to that processing.",
      ...
      if (c.modelTrainingRequired) {
        const ok = await confirmDialog({
          ...
          body: "On the free plan, your uploaded documents may be used to help improve DocAIQ's AI models (for example, learning better field schemas). Paid plans keep your data private and are never used for training. By continuing on the free plan you consent to this use.",
      ...
      <div ref={scroller} className="bg2 border rounded-md" style={{ flex: "1 1 0", overflowY: "auto", overflowX: "hidden", minWidth: 0, padding: 12, marginBottom: 10, minHeight: mobile ? 0 : 200 }}>
        {messages.length === 0 && !busy ? (
          ...
          <div key={m.id} style={{ marginBottom: 12, textAlign: m.role === "user" ? "right" : "left" }}>
            <div className="upper ink4" style={{ fontSize: 9, letterSpacing: ".08em", marginBottom: 3 }}>{m.role === "user" ? "You" : "DocAIQ"}</div>
